...
- **Claude Code**: `claude-alert` plays a sound + notification when Claude wants input and its terminal isn't focused; sounds live in `~/.local/share/sounds/`.
  ...
  It lives here and not in `shared-datafiles` because that repo auto-squashes and force-pushes its own history, which has silently reverted the file before.
  The `[CAVEMAN]`/`[PONYTAIL]` badge scripts it calls stay in `~/.claude/hooks` — those are vendored by their plugins. The rest of the config is **not** here — see below.
